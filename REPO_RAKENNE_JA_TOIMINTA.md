...
- `core.projects`: hankkeet, esimerkiksi Kilpikoivu, Hellemäentie, Lemmikkitie
...
Työn aikana Hellemäentien, Kilpikoivun ja Lemmikkitien lisäaineistoista on alettu mallintaa oikean työmaan etenemistä: kokoukset, vastaanotto, lisätyöt, käytännön ongelmat, valvojan havainnot ja ratkaisut.
...
Nykyinen huomio: redaktoinnissa on vielä projektikohtaisia kovakoodattuja poistoja erityisesti Kilpikoivun aineistolle. Se kannattaa myöhemmin yleistää.
...
Tämä on vanhempi ja osittain Kilpikoivu-painotteinen rikastaja. Se sisältää paljon käsityönä rakennettua tulkintaa, kuten:
...
Se sisältää myös erikoislogiikkaa esimerkiksi Lemmikkitien kaltaisille kohteille, joissa rakennuksia on useita ja pystylinjojen määrä voidaan ilmaista vaihteluvälinä.
...
3. Jos sopivaa referenssiä ei löydy, käytetään Kilpikoivua oletuksena.
...
- Kilpikoivu on default-referenssi hyvin onnistuneesta limisukitusurakasta
...
```powershell
cipp-inventory-source-files --project-code pilot_001 --source-dir data/raw/pilot_001
```
...
```powershell
cipp-extract-pdf-pages --project-code pilot_001 --output-dir data/extracted/pilot_001/pages
```
...
```powershell
cipp-build-markdown --project-code pilot_001 --output-dir data/extracted/pilot_001/markdown
```
...
```powershell
cipp-build-canonical --project-code pilot_001 --output data/normalized/pilot_001/canonical.json
```
...
```powershell
cipp-build-project-canonical --project-code pilot_001 --output data/normalized/pilot_001/canonical.json
```

### `cipp-enrich-canonical`

Rikastaa canonical JSON -mallia markdownista. Tämä on hyödyllinen erityisesti vanhemman Kilpikoivu-putken kanssa.

```powershell
cipp-enrich-canonical --input data/normalized/pilot_001/canonical.json --markdown-dir data/extracted/pilot_001/markdown --output data/normalized/pilot_001/canonical.enriched.json
```
...
```powershell
cipp-validate-canonical --input data/normalized/pilot_001/canonical.json --report data/reports/pilot_001.validation.json
```
...
```powershell
cipp-load-contract --input data/normalized/pilot_001/canonical.json
```
...
```powershell
cipp-link-contract-documents --project-code pilot_001
```
...
```powershell
cipp-load-markdown-sections --project-code pilot_001 --markdown-dir data/extracted/pilot_001/markdown
```
...
- Kilpikoivussa urakkasopimus voi antaa hinnan ja maksuerät.
...
- Kilpikoivu toimii oletusreferenssinä
- pieni taloyhtiö liikkuu lähemmäs korkeaa euroa/asunto-hintaa
- lähin sisäinen referenssi valitaan ennen oletusreferenssiä

### `tests/eval_questions/pilot_001.jsonl`

Sisältää ensimmäiset arviointikysymykset Kilpikoivun aineistolle. Näiden avulla voidaan myöhemmin testata, osaako hakukerros löytää oikean lähdeasiakirjan.
...
Hellemäentien, Kilpikoivun ja Lemmikkitien lisäaineistot osoittivat, että työmaan kokoukset, vastaanotto, lisätyöt, maksut ja käytännön ongelmat ovat erittäin arvokkaita.
...
`enrich_canonical_from_markdown.py` sisältää paljon arvokasta Kilpikoivu-tulkintaa, mutta `build_project_canonical.py` on yleisempi usean projektin malli.
...
4. Jos dataa puuttuu, Kilpikoivu toimii oletusreferenssinä.
