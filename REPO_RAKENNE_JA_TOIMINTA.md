...
Maksuerät ovat oma tarkistuskerroksensa. `cipp-report-reference-facts` hyväksyy rakenteisen maksuerävertailun, kun `finance.payment_schedule_items`-rivien luotettava summa vastaa sopimushintaa enintään yhden euron pyöristystoleranssilla. Raportti näyttää erotuksen kentissä `payment_schedule_difference` ja `payment_schedule_difference_pct`. Jos maksueräasiakirja on olemassa mutta rivejä ei ole purettu rakenteiseksi, `payment_schedule_evidence_status` on `document_present_not_structured` ja projekti jää `needs_review`-tilaan. Jos maksueräasiakirjaa ei ole linkitetty eikä rivejä ole, tila on `no_payment_schedule_source`; puute raportoidaan mutta se ei yksin blokkaa valmiutta. `next_blocker` näyttää ensimmäisen seuraavaksi korjattavan portin.
