...
`payment_schedule_evidence_status` kertoo maksuerien tilan: `structured_and_matches`, `structured_but_mismatch`, `found_unstructured` tai `not_found`. `found_unstructured` tarkoittaa, että maksuerätaulukko tai maksuerämaininta löytyy tekstistä, mutta rivejä ei vielä voida poimia luotettavasti. `not_found` tarkoittaa nykyisen discovery-logiikan puutetta eikä todista, ettei aineistossa olisi taulukkoa. Knowledge graphia ei aloiteta ennen kuin maksuerätaulukot on rakenteistettu riittävän hyvin.
