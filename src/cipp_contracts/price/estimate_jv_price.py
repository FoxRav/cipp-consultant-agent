...
REFERENCE_PROJECT_CODE = "reference_001"
REFERENCE_PROJECT_LABEL = "Referenssikohde A"
...
BASELINE_PRICE_PER_APARTMENT = REFERENCE_PRICE_GROSS / REFERENCE_APARTMENTS
...
def estimate_jv_price(
    ...
    reference = select_reference_project(
        references or [baseline_reference()],
    ...
    return JvPriceEstimate(
        ...
        compared_reference_count=len(references or [baseline_reference()]),
        assumptions={
            "currency": "EUR",
...
def baseline_reference() -> ReferenceProject:
...
def select_reference_project(
    ...
    candidates = references or [baseline_reference()]
    matches = [
        _score_reference(
            reference,
            apartment_count=apartment_count,
...
def fetch_reference_projects(db_url: str) -> list[ReferenceProject]:
    ...
    return references or [baseline_reference()]
...
def main() -> None:
    ...
    references = fetch_reference_projects(db_url) if db_url else [baseline_reference()]
